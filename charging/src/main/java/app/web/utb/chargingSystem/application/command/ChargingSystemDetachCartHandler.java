...
@Transactional
@AllArgsConstructor
public class ChargingSystemDetachCartHandler implements CmdHandler<ChargingSystemDetachCartCmd> {
    private final ChargingSystemRepository chargingSystemRepository;
    private final DomainEventPublisher domainEventPublisher;

    @Override
    public void handle(ChargingSystemDetachCartCmd command) {
        ChargingSystem chargingSystem = chargingSystemRepository.findByIdOrThrowException(command.getSystemId());
        chargingSystem.detachCart(command.getChargerPosition(), command.getCartId());
